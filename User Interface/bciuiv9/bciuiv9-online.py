...
#self.text_input = text[targetRow][targetCol]
...
class MyOVBox(OVBox):
    ...
    def initialize(self): 
        ...
        with open('target_stims.txt') as f:
        ...
        self.widget = user_input.TextWidget('', 0, int(7*height/12), int(width),int(5*height/12), self.batch)
        self.text_input = ""
        self.widget.caret.on_text(self.text_input)
    ...
    #def checkTarget(self, targetRow,targetCol):
    #        if stimulationCol == target[0] and stimulatonRow == target[1]:
                #send target = true
    #        else:
    #           #send target = false
    ...
    def process(self): # Called on each box clock tick (this can be configured by right-clicking the box)
        ...
        if self.win.has_exit:
            ...
            #untested
            #if(self.input[0] == self.target[0] and self.input[1] == self.target[1]):
            #for displaying output
            #self.text_input = text[targetRow][targetCol]
            #self.widget.caret.on_text(self.text_input)
            #self.text_input = ""
