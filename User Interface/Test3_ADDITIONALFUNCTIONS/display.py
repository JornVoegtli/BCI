...
class MyPyglet():
    ...
    def drawCircleFlash(self, rowcol):
        ...
        if (rowcol < 6 and isDrawVertCircle):
            for j in range (0,6):
                ypos  = j*(keyboardPositionTop)/5 + circleRadius/2
                xpos  = rowcol*width/6 + circleRadius/2 
            ...
            for j in range (0,6):
                ypos  = rowcol%6*(keyboardPositionTop)/5 + circleRadius/2
                xpos  = j*width/6 + circleRadius/2
    ...
    def makeSelection(self, selection, selectionStr):
        ...
        # Backspace
            ...
            self.widget.caret.on_text_motion(pyglet.window.key.MOTION_BACKSPACE)
        elif (self.text_input == "ENTER"): 
            self.current_text = self.current_text + "(ENTER)" 
            self.widget.caret.on_text_motion("(ENTER)")
