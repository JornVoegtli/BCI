...
class MyOVBox(OVBox):
    ...
    def drawTarget(self, targetRow, targetCol):
        if(isDrawTarget):
            primitives.drawRect(targetRow*width/6, targetCol*height/12, targetSize[0], targetSize[1], targetColour[0],targetColour[1],targetColour[2],targetColour[3])
        return
    ...
    def process(self): # Called on each box clock tick (this can be configured by right-clicking the box)
        ...
        if self.win.has_exit:
            ...
            if (self.loopCounter <= 10):
                ...
                self.drawTarget(self.target[0]-OVTK_StimulationId_Label_00, self.target[1]-OVTK_StimulationId_Label_06)
