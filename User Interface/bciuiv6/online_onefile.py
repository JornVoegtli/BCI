...
class MyOVBox(OVBox): #MyOVBox(): # MyOVBox(OVBox):
    ...
    def initialize(self): 
        ...
        self.inputList = [0,0,0,0] # Prepare for four inputs
        return
    ...
    def process(self): 
        ...
        ## Read inputs
        self.updateInputs()
        flash = self.inputList[0]
        target = self.inputList[1]
        row_ans = self.inputList[2]
        col_ans = self.inputList[3]
        ...
        if (self.clkdiv == 0):
            ...
            self.rowcol = random.randint(0,11)
        self.dp.drawP300(self.rowcol, 0, self.caps_on)
        # Send row/col flashes
        self.sendOutput(0, STIMSTART)
        self.sendOutput(0, STIMLABELBASE+self.rowcol)
...
#        self.sendOutput(2, self.SSVEP_target)
        ...
        self.updateTextInput(row_ans, col_ans)
    ...
    def matrix_to_key(self, row, col):
        textUC = [ ['A','B','C','D','E',u"\u2190"], 
                ['F','G','H','I','J','ENTER'], 
                ['K','L','M','N','O','abc'], 
                ['P','Q','R','S','T','123'], 
    ...
    def updateTextInput(self, row_ans, col_ans):
        ## Get new character
        keyname = self.matrix_to_key(row_ans, col_ans)
        if (keyname != self.key): # Quick fix: update only when input changes. Change later!
            self.key = keyname
            if (self.keepWriting):
